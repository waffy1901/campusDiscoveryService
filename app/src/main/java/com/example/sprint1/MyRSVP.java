...
public class MyRSVP extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_my_rsvp);
    ...
    public static List<JSONObject> sendGETRequest() {
        ...
        try {
            URL url = new URL("https://campusdiscovery.herokuapp.com/events/myRSVP");
            connection = (HttpURLConnection) url.openConnection();

            connection.setRequestMethod("GET");
            connection.setRequestProperty("Authorization", "Bearer " + currUser.getToken());
            int status = connection.getResponseCode();
            System.out.println(status + " myRSVP");
            if (status > 299) {
    ...
    private void populateEventList() {
        ...
        RecyclerView recycler = findViewById(R.id.rcyRSVP);
